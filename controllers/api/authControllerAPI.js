...
module.exports = {
  ...
  forgotPassword: function(req,res,next){
    Usuario.findOne({email: req.body.email}, function(err, usuario){
      ...
      usuario.setPassword(function(err){
        if(err){ return next(err) }
        res.status(200).json({
          message: "Se envió un email para reestablecer la contraseña",
          data: null
        })
      })
    })
  ...
  authFacebookToken: function(req, res, next){
    ...
    if(req.user){
      req.user.save().then(() => {
        const token = jwt.sign({id: req.user.id}, process.env.JWT_KEY, {expiresIn: "7d"})
